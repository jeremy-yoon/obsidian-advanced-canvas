...
export default class CanvasHelper {
	...
	static createCardMenuOption(
		canvas: Canvas,
		menuOption: MenuOption,
		previewNodeSize: () => Size,
		onPlaced: (canvas: Canvas, pos: Position) => void
	): HTMLElement {
		...
		menuOptionElement.addEventListener("click", (_e) => {
			onPlaced(
				canvas,
				this.getCenterCoordinates(canvas, previewNodeSize())
			);
		});
		...
		const menuOptionElement = this.createPopupMenuOption({
			...
			callback: () => {
				...
				if (menuOptionElement.classList.contains("is-active")) {
					...
					menuOptionElement.parentElement
						?.querySelector(`#${submenuId}`)
						?.remove();
					return;
				}
				...
				for (const subMenuOption of subMenuOptions) {
					const subMenuOptionElement =
						this.createPopupMenuOption(subMenuOption);
	...
	static addPopupMenuOption(
		...
		const sisterElement =
			index >= 0
				? popupMenuEl.children[index]
				: popupMenuEl.children[popupMenuEl.children.length + index];
	...
	static zoomToBBox(canvas: Canvas, bbox: BBox) {
		...
		const zoomedBBox = BBoxHelper.scaleBBox(
			bbox,
	...
	static createRunButton(canvas: Canvas, run: () => void) {
		const RUN_BUTTON_ID = "run-button";
		const RUN_BUTTON_ICON = "play";

		// Check if run button exists
		const runButtonElement = canvas?.menu?.menuEl?.querySelector(
			`#${RUN_BUTTON_ID}`
		);
		...
		runButton.classList.add("clickable-icon");
		setIcon(runButton, RUN_BUTTON_ICON);
		setTooltip(runButton, "Run", { placement: "top" });

		runButton.addEventListener("click", run);

		canvas.menu.menuEl.appendChild(runButton);
	}

	static createStyleDropdownMenu(
		canvas: Canvas,
		stylableAttributes: StyleAttribute[],
		currentStyleAttributes: { [key: string]: string | null },
		setStyleAttribute: (
		...
		styleMenuButtonElement.addEventListener("click", () => {
			const isOpen =
				styleMenuButtonElement.classList.toggle("has-active-menu");
			if (!isOpen) {
				popupMenuElement
					.querySelector(`#${STYLE_MENU_DROPDOWN_ID}`)
					?.remove();
				popupMenuElement
					.querySelector(`#${STYLE_MENU_DROPDOWN_SUBMENU_ID}`)
					?.remove();
			...
			styleMenuDropdownElement.style.top = `${
				popupMenuElement.getBoundingClientRect().height
			}px`;

			const canvasWrapperCenterX =
				canvas.wrapperEl.getBoundingClientRect().left +
				canvas.wrapperEl.getBoundingClientRect().width / 2;

			const leftPosition =
				styleMenuButtonElement.getBoundingClientRect().left -
				popupMenuElement.getBoundingClientRect().left;
			const rightPosition =
				popupMenuElement.getBoundingClientRect().right -
				styleMenuButtonElement.getBoundingClientRect().right;

			// Swap sides if it is too close to the edge
			if (
				popupMenuElement.getBoundingClientRect().left + leftPosition <
				canvasWrapperCenterX
			)
			...
			for (const stylableAttribute of stylableAttributes) {
				...
				const selectedStyle =
					stylableAttribute.options.find(
						(option) =>
							currentStyleAttributes[
								stylableAttribute.datasetKey
							] === option.value
					) ??
					stylableAttribute.options.find(
				...
				// Add hover effect
				stylableAttributeElement.addEventListener(
					"pointerenter",
					() => {
				...
				stylableAttributeElement.addEventListener("click", () => {
					...
					const styleMenuDropdownSubmenuElement =
						document.createElement("div");
					styleMenuDropdownSubmenuElement.id =
						STYLE_MENU_DROPDOWN_SUBMENU_ID;
					...
					const topOffset =
						parseFloat(
							window
								.getComputedStyle(styleMenuDropdownElement)
								.getPropertyValue("padding-top")
						) +
						(styleMenuDropdownElement.offsetHeight -
							styleMenuDropdownElement.clientHeight) /
							2;
					styleMenuDropdownSubmenuElement.style.top = `${
						stylableAttributeElement.getBoundingClientRect().top -
						topOffset -
						popupMenuElement.getBoundingClientRect().top
					}px`;

					// Swap sides if it is too close to the edge
					const leftPosition =
						styleMenuDropdownElement.getBoundingClientRect().right -
						popupMenuElement.getBoundingClientRect().left;
					const rightPosition =
						popupMenuElement.getBoundingClientRect().right -
						styleMenuDropdownElement.getBoundingClientRect().left;
					...
					if (
						...
						styleMenuDropdownSubmenuElement.style.right = `${rightPosition}px`;

					// Add style options
					for (const value of stylableAttribute.options) {
						const styleMenuDropdownSubmenuOptionElement =
							document.createElement("div");
						styleMenuDropdownSubmenuOptionElement.classList.add(
							"menu-item"
						);
						styleMenuDropdownSubmenuOptionElement.classList.add(
							"tappable"
						...
						// Add icon
						const submenuIconElement =
							document.createElement("div");
						submenuIconElement.classList.add("menu-item-icon");
						setIcon(submenuIconElement, value.icon);
						styleMenuDropdownSubmenuOptionElement.appendChild(
							submenuIconElement
						);

						// Add label
						const submenuLabelElement =
							document.createElement("div");
						...
						styleMenuDropdownSubmenuElement.appendChild(
							styleMenuDropdownSubmenuOptionElement
						);

						// Add hover effect
						styleMenuDropdownSubmenuOptionElement.addEventListener(
							"pointerenter",
							() => {
								styleMenuDropdownSubmenuOptionElement.classList.add(
									"selected"
						...
						styleMenuDropdownSubmenuOptionElement.addEventListener(
							"pointerleave",
							() => {
								styleMenuDropdownSubmenuOptionElement.classList.remove(
									"selected"
								);
							}
						);
						...
						styleMenuDropdownSubmenuOptionElement.addEventListener(
							...
							() => {
								...
								// Close menu
								styleMenuButtonElement.dispatchEvent(
									new Event("click")
								);
							}
						);
...
export const createNode = (
	...
	if (nodeData) {
		// nodeData가 Partial<CanvasNodeData>이므로 타입 검사 및 변환
