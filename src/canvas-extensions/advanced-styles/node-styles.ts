...
import { noteGenerator } from "src/utils/llm-helper";
...
export default class NodeStylesExtension extends CanvasExtension {
	...
	isEnabled() {
		return "nodeStylingFeatureEnabled" as const;
	}
	...
	private onPopupMenuCreated(canvas: Canvas): void {
		...
		if (
			canvas.readonly ||
			selectionNodeData.length === 0 ||
			selectionNodeData.length !== canvas.selection.size
		)
			return;

		const selectedNodeTypes = new Set(
			selectionNodeData.map((node) => node.type)
		);
		const availableNodeStyles = this.allNodeStyles.filter(
			(style) =>
				!style.nodeTypes ||
				style.nodeTypes.some((type) => selectedNodeTypes.has(type))
		);

		CanvasHelper.createRunButton(canvas, () => {
			const app = this.plugin.app;
			const fromNode = selectionNodeData[0];
		...
		CanvasHelper.createStyleDropdownMenu(
			...
			(attribute, value) =>
				this.setStyleAttributeForSelection(canvas, attribute, value)
		);
	}

	private setStyleAttributeForSelection(
		canvas: Canvas,
		...
		for (const nodeData of selectionNodeData) {
			...
			if (
				attribute.nodeTypes &&
				!attribute.nodeTypes.includes(nodeData.type)
			)
				continue;
